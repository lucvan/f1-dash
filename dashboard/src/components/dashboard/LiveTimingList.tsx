...
// The compact live timing leaderboard shown alongside the Track Map and the
// F1 TV stream. Extracted from track-map so both pages share one implementation.
export default function LiveTimingList() {
				...
				<AnimatePresence>
					{Object.values(driversTiming.Lines)
						...
						.map((timingDriver, index) => (
							<TimingListDriver
								key={`timinglist.driver.${timingDriver.RacingNumber}`}
								position={index + 1}
...
const TimingListDriver = ({ position, driver, timingDriver }: TimingListDriverProps) => {
